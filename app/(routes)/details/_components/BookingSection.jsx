...
function BookingSection({children,business}) {
    ...
    const BusinessBookedSlot = () => {
        GlobalApi.BusinessBookedSlot(business?.id, moment(date).format('DD-MMM-yyyy'))
            .then(resp => {
                console.log(resp);
                if (Array.isArray(resp.bookings)) {
                    setBookedSlot(resp.bookings);
                } else {
                    console.error("Booking data is not an array:", resp.bookings);
                }
            })
            .catch(error => {
    ...
    const getTime = () => {
      ...
      const saveBooking=()=>{
            GlobalApi.createNewBooking(business.id,
                ...
                .then(resp=>{
                    ...
                    //Error Toast Msg
                    Swal.fire({
                        title: 'Error while creating booking!',
                        icon: 'error',
                        showConfirmButton: false,
                        showCancelButton: false,
                        timer: 2000,
      ...
      const isSlotBooked = (time) => {
        // Ensure bookedSlot is an array before using find
